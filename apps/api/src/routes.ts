...
import { DeployRequestSchema, ProjectSchema, scoreClawPackage, scoreHardwareForTemplate } from "@lumiforge/core";
...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.get("/devices/:id", async (request, reply) => {
    ...
    return device ?? reply.code(404).send({ message: "Device not found" });
  });
  ...
  app.get("/templates/:id", async (request, reply) => {
    ...
    return template ?? reply.code(404).send({ message: "Template not found" });
  });
  app.get("/skills", async () => skillCatalog);
  app.get("/skills/:id", async (request, reply) => {
    const { id } = request.params as { id: string };
    const skill = skillCatalog.find((item) => item.id === id);
    return skill ?? reply.code(404).send({ message: "Skill not found" });
  });
  ...
  app.get("/runtime/blueprint/:id", async (request, reply) => {
    ...
    return blueprint ?? reply.code(404).send({ message: "Runtime blueprint not found" });
  });
  ...
  app.get("/recommendations/:templateId", async (request, reply) => {
    ...
    return devices.map((device) => ({ device, score: scoreHardwareForTemplate(device, template) })).sort((a, b) => b.score - a.score);
  });
  ...
  app.get("/projects/:id/agent-config", async (request, reply) => {
    ...
    return { agent: compileProjectToAgentConfig(project), runtime: getDefaultRuntime() };
  });
